...
function templateCard(key,t,custom=false){return `<button class="label-template-card ${labelDesigner.templateKey===key?"active":""}" data-template-key="${esc(key)}" data-custom="${custom?"1":"0"}"><div class="template-miniature"><span></span><b>${esc(t.name)}</b><i>QR</i></div><strong>${esc(t.name)}</strong><small>${esc(t.description||`${t.widthMm} × ${t.lengthMm} mm`)}</small><em>${t.widthMm} × ${t.lengthMm} mm</em></button>`}
function renderLabelCenter(){
  currentTab="labels";const saved=loadSavedTemplates();
  app.innerHTML=shell(`<section class="label-center-header"><div><p class="eyebrow">Production workspace</p><h1>Label Center</h1><p>Design, batch, and print professional labels for the Brother QL-820NWB.</p></div><div class="printer-status-card"><span class="status-dot"></span><div><strong>${QL820_PROFILE.name}</strong><small>System print · 62 mm continuous roll</small></div><button id="printer-help" class="secondary compact">Setup</button></div></section>
  <section class="label-center-metrics"><article><span>Printer profile</span><strong>QL-820NWB</strong><small>300 dpi · cutter ready</small></article><article><span>Plants available</span><strong>${plants.length}</strong><small>Ready for labels</small></article><article><span>Templates</span><strong>${Object.keys(LABEL_TEMPLATES).length+saved.length}</strong><small>${saved.length} custom</small></article><article><span>Media</span><strong>62 mm</strong><small>Continuous roll</small></article></section>
  <section class="dashboard-section"><div class="section-heading"><div><p class="eyebrow">Create</p><h2>Start a label job</h2></div><button id="batch-label-action" class="secondary">Batch print</button></div><div class="label-template-grid">${Object.entries(LABEL_TEMPLATES).map(([k,t])=>templateCard(k,t)).join("")}${saved.map((t,i)=>templateCard(`custom-${i}`,t,true)).join("")}</div></section>
  <section class="label-center-lower"><article class="label-quick-panel"><div><p class="eyebrow">Quick print</p><h2>Recent plants</h2></div><div class="label-plant-list">${plants.slice(0,8).map(p=>`<button data-label-plant="${esc(p.accession)}"><span><strong>${esc(p.name)}</strong><small>${esc(p.accession)} · ${esc(val(p,["location","support"],"Unassigned"))}</small></span><b>Design ›</b></button>`).join("")||'<div class="empty-state">Add a plant to begin.</div>'}</div></article><article class="print-guide-card"><p class="eyebrow">Printing</p><h2>One-tap system workflow</h2><ol><li>Design the label</li><li>Tap Print</li><li>Select the QL-820NWB in the iOS/macOS print dialog</li><li>Confirm the installed roll and print</li></ol><button id="single-label-action" class="primary">Choose a plant</button></article></section>`,"labels");
  bindShell();
  document.querySelector("#single-label-action").onclick=openPlantPickerForLabel;
  ...
  document.querySelectorAll("[data-label-plant]").forEach(b=>b.onclick=()=>openPlantLabelDesigner(plants.find(p=>p.accession===b.dataset.labelPlant)));
  document.querySelectorAll("[data-template-key]").forEach(b=>b.onclick=()=>{const savedNow=loadSavedTemplates();const custom=b.dataset.custom==="1"?savedNow[Number(b.dataset.templateKey.split("-").pop())]:null;labelDesigner.templateKey=b.dataset.templateKey;labelDesigner.template=cloneTemplate(custom||LABEL_TEMPLATES[b.dataset.templateKey]);openPlantPickerForLabel()});
}
...
function openPlantLabelDesigner(p){
  ...
  labelDesigner.selectedId=null;labelDesigner.history=[];labelDesigner.future=[];
  modalRoot.innerHTML=`<div class="label-studio-backdrop"><section class="label-studio"><header class="label-studio-header"><div><button id="close-studio" class="icon-button">×</button><span><strong>Label Studio</strong><small>${esc(p.name)} · ${esc(p.accession)}</small></span></div><div class="studio-actions"><button id="undo-label" class="ghost compact">↶</button><button id="redo-label" class="ghost compact">↷</button><button id="save-template" class="secondary compact">Save template</button><button id="export-label" class="secondary compact">PNG</button><button id="print-label-now" class="primary compact">Print</button></div></header><div id="designer-workspace"></div></section></div>`;
  document.querySelector("#close-studio").onclick=closeModal;document.querySelector("#undo-label").onclick=undoLabel;document.querySelector("#redo-label").onclick=redoLabel;document.querySelector("#save-template").onclick=saveCustomTemplate;document.querySelector("#export-label").onclick=exportCurrentLabel;document.querySelector("#print-label-now").onclick=printCurrentLabel;renderDesignerWorkspace();
}
...
function bindDesignerEvents(){
 ...
 document.querySelectorAll("[data-element-id]").forEach(node=>{node.onpointerdown=e=>startElementDrag(e,node.dataset.elementId);node.onclick=e=>{e.stopPropagation();labelDesigner.selectedId=node.dataset.elementId;renderDesignerWorkspace()}});
