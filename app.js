...
function openPlantLabelDesigner(p){
  ...
  labelDesigner.selectedId=null;labelDesigner.history=[];labelDesigner.future=[];mobileStudioPanel=null;
  modalRoot.innerHTML=`<div class="label-studio-backdrop"><section class="label-studio"><header class="label-studio-header"><div class="studio-title-group"><button id="close-studio" class="icon-button" aria-label="Close">×</button><span><strong>Label Studio</strong><small>${esc(p.name)} · ${esc(p.accession)}</small></span></div><div class="studio-actions"><button id="undo-label" class="ghost compact" aria-label="Undo">↶</button><button id="redo-label" class="ghost compact" aria-label="Redo">↷</button><button id="save-template" class="secondary compact action-save">Save</button><button id="export-label" class="secondary compact action-export">PNG</button><button id="print-label-now" class="primary compact action-print">Print</button></div></header><div id="designer-workspace"></div></section></div>`;
